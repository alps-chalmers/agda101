...
module Hoare {-everything from here to 'where' are arguments for when you're importing the module-}
    (Cond : Set) {-conditions. not sure what kind-}
    (PrimComm : Set) {-?-}
    (neg : Cond -> Cond) {-negation function-}
    (_/\_ : Cond -> Cond -> Cond) {-and function-}
    (Tautology : Cond -> Cond -> Set) {-rule of inference-}
    (State : Set) {-state... of what?-}
    (SemCond : Cond -> Pred State) {-some kind of condition function, what is pred?-}
    (tautValid : (b1 b2 : Cond) -> Tautology b1 b2 -> 
                 (s : State) -> SemCond b1 s -> SemCond b2 s) {-?-}
    (respNeg : (b : Cond) -> (s : State) -> 
               Iff (SemCond (neg b) s) (¬ SemCond b s)) {-not sure what Iff does. maybe checks if semCond of the negation of condition and state is same as/equivalent to not semCond of condition and state-}
    (respAnd : (b1 b2 : Cond) -> (s : State) ->
               Iff (SemCond (b1 /\ b2) s)
                   ((SemCond b1 s) × (SemCond b2 s))) {-same assumption as above. maybe returns if SemCond of cond1 and cond2 is same as/equivalent to the union of both SemConds-}
    (PrimSemComm : ∀ {l} -> PrimComm -> Rel State l) {-?-}
    (Axiom : Cond -> PrimComm -> Cond -> Set) {-some kind of axiom I guess-}
