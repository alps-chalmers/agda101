...
p0=>s1 : {p : Prog (s 0)} → p ⊨ (at (s 1))
p0=>s1 {p} = flow (∧-e₁ (:=n-R (init p) s0)) s0

p0=>s2 : {p : Prog (s 0)} → p ⊨ (at (s 2))
p0=>s2 = flow (∧-e₁ (:=n-R p0=>s1 s1)) s1

p0=>s3∧s4 : {p : Prog (s 0)} → p ⊨ ((at (s 3)) ∧ (at (s 4)))
p0=>s3∧s4 = parRule p0=>s2 s2

p0=>s3' : {p : Prog (s 0)} → p ⊨ (after (s 3))
p0=>s3' = ∧-e₁ (:=n-R (∧-e₁ p0=>s3∧s4) s3)

p0=>s4' : {p : Prog (s 0)} → p ⊨ (after (s 4))
p0=>s4' = ∧-e₁ (:=n-R (∧-e₂ p0=>s3∧s4) s4)

-- Final proof of termination
p0=>s2' : {p : Prog (s 0)} → p ⊨ (after (s 2))
p0=>s2' = exitPar (∧-i p0=>s3' p0=>s4') s2

-- Proof of ◇ x==5
p0=>x==5 : {p : Prog (s 0)} → p ⊨ ("x" ==n (nat 5))
...
w0=>w1 : {p : Prog (s 0)} → p ⊨ (at (s 1))
w0=>w1 {p} = flow (∧-e₁ (:=n-R (init p) w0)) w0

w1=>w2∧w3 : {p : Prog (s 0)} → p ⊨ (at (s 1)) → p ⊨ ((at (s 2)) ∧ (at (s 3)))
w1=>w2∧w3 p = parRule p w1

w2∧w3=>w2'∧w3' : {p : Prog (s 0)} → p ⊨ ((at (s 2)) ∧ (at (s 3))) → p ⊨ ((after (s 2)) ∧ (after (s 3)))
w2∧w3=>w2'∧w3' p = ∧-i (∧-e₁ (:=n-R (∧-e₁ p) w2)) (exWhile-F (∧-e₂ p) w3)

w2'∧w3'=>w5 : {p : Prog (s 0)} → p ⊨ ((after (s 2)) ∧ (after (s 3))) → p ⊨ (at (s 5))
w2'∧w3'=>w5 p = flow (exitPar (w2∧w3=>w2'∧w3' (w1=>w2∧w3 w0=>w1)) w1) w1

w5=>w5' : {p : Prog (s 0)} → p ⊨ (at (s 5)) → p ⊨ (after (s 5))
w5=>w5' p = ∧-e₁ (:=n-R (w2'∧w3'=>w5 (w2∧w3=>w2'∧w3' (w1=>w2∧w3 w0=>w1))) w5)

-- Proof of termination and property of variable "x"
s0=>x==5∧s5' : {p : Prog (s 0)} → p ⊨ (at (s 0)) → p ⊨ (("x" ==n (nat 5)) ∧ (after (s 5)))
...
p=>a2∧a3 : {p : Prog (s 0)} → p ⊨ ((at (s 2)) ∧ (at (s 3)))
p=>a2∧a3 {p} = parRule (flow (∧-e₁ (:=b-T-R (init p) a0)) a0) a1

a2=>□~x : {p : Prog (s 0)} → p ⊨ (□ (∼ (tr (var "x"))))
a2=>□~x = □-∧-e₂ (⇒-e (custom ((after (s 2)) ∧ (∼ (tr (var "x")))) (□ ((after (s 2)) ∧ ∼ (tr (var "x"))))) (:=b-F-R (∧-e₁ p=>a2∧a3) a2))

p=>a2'∧a3' : {p : Prog (s 0)} → p ⊨ ((after (s 2)) ∧ (after (s 3)))
p=>a2'∧a3' = ∧-i (∧-e₁ (:=b-F-R (∧-e₁ p=>a2∧a3) a2)) (exitWhile (∧-i (∧-e₂ p=>a2∧a3) a2=>□~x) a3)

p=>a1' : {p : Prog (s 0)} → p ⊨ (after (s 1))
p=>a1' = exitPar p=>a2'∧a3' a1

-- Final proof of Termination

p=>x==6∧a5' : {p : Prog (s 0)} → p ⊨ (("x" ==n (nat 6)) ∧ (after (s 5)))
